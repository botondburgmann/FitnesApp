...
const Toplist = ({navigation}: RouterProps) => {
  ...
  function sortUsers (users: User[]): User[] {
    ...
    for (let i = 0; i < n - 1; i++)
      for (let j = 0; j < n - i - 1; j++)
        if (sortedUsers[j].weeklyExperience < sortedUsers[j + 1].weeklyExperience) [sortedUsers[j], sortedUsers[j + 1]] = [sortedUsers[j + 1], sortedUsers[j]];
  ...
  function selectLoggedInUser(users:User[], userID: string | null) : User {
    ...
    for (const user of users)
      if (user.userID === userID)loggedInUser = user;
  ...
  function selectSimilarUsers (users: User[], loggedInUser: User): User[] {
    ...
    for (const user of users) {
      ...
      if (user.gender === loggedInUser.gender && userBMI >= loggedInUserBMI-5 && userBMI <= loggedInUserBMI+ 5)similarUsers.push(user);
    }
  ...
  useEffect(() => {
    const unsubscribeFromUers = getAllUsers((users: User[]) => {
      ...
      if (sortedUsers.length > 10) sortedUsers.splice(10,sortedUsers.length-10)
