...
const DisplayStraightSet = (props) => {
    ...
    if (weightsNumber > 1) 
      ...
      let samerest = true;
      const restNumber = new Set(sets.rest).size;
      if (restNumber > 1) 
      samerest = false;
  ...
  return (
    <View>
      ...
      {noWeight ? <View style={{ flexDirection: 'row' }}>
          {sets.map((item, index) => (
            <Text  key={index}>{item.reps} </Text>
          ))}
          <Text>repetitions with no weight </Text>
      </View>
      : sameWeight ? <View style={{ flexDirection: 'row' }}>
      {sets.map((item, index) => (
        <Text  key={index}>{item.reps} </Text>
      ))}
      <Text>repetitions with {sets[0].weight} kg each </Text>
          ...
          {sets.map((item, index) => (
            <Text  key={index}>{item.weight} kg </Text>
          ))}
      </View>}
      
      {samerest ? <View style={{ flexDirection: 'row' }}>
        <Text>Rest: {sets[0].restTime/60} minutes between sets</Text>
  </View> : <View style={{ flexDirection: 'row' }}>
  <Text>Rest: </Text>

          {sets.map((item, index) => (
            <Text  key={index}>{item.restTime/60} minutes</Text>
          ))}
      </View>}
    </View>
  )
